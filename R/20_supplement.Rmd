...
Visual inspection suggested that accuracy, F1 and possibly balanced accuracy were affected by the data balance.
...
As additional covariates, the number of interactions $n_{int}$ and the sequence length $n_{seq}$ (numerical) and the fold number $k_{fold}$ (categorical).
The `strategy` variable was tested with a type 3 ANOVA, being significant with $p < 0.05$ for `acc`, `f1` and `balanced_acc` (table \@ref(tab:anova-foldavg-strat)). 
...
df.anova.baseline %>% subset(variable == "strategy") %>%
  knitr::kable(format = "latex", 
               ...
               booktabs = TRUE, label = "anova-foldavg-strat") %>%
    kableExtra::kable_styling(latex_options = c("scale_down", "hold_position"))
```

Based on this, metrics were divided in two types:

* Those where the baseline is different between strategies, i.e. imbalance-sensitive: `acc`, `f1` and `balanced_acc`. Therefore, before comparing strategies, the baseline performance needed to be accounted for.
