---
title: "Appendix 5: model predictions and performance (PRs)"
author: 
...
df.clean <- read.csv(config$file.cleanperf.pr) %>%
...
This supplement describes the behaviour of the proteochemometrics (PCM) deep learning model to predict protein-compound bioactivity for \textbf{PRs}. 
...
* `no_resampling` was noticeably \textbf{inclined to predict more positives than negatives, and in general more extreme ratios than the real ones}.
* `resampling_before_clustering` and `semi_resampling` \textbf{alleviated the imbalance in the predictions, being was quite close to the actual distribution}.
...
* `resampling_before_clustering` showed a \textbf{negative but not significant coefficient in PRs (p=0.073)}. 
...
* `n_interactions`: \textbf{the term is not significant in PRs}.
...
* \textbf{This model confirms both conclusions from the model} in the `semi_resampling` strategy. \textbf{The coefficient for ratio test is similar and significant in both models; the coefficient of ratio training is non-significant in both models.}
...
* Although the \textbf{estimate is larger and more significant} for `training_ratio`, the \textbf{confounding effect of the predicted ratios deems this model inconclusive}. \textbf{For PRs the distribution of predicted ratios is not very skewed}.
...
\textbf{All of them apply to PRs as well, except for the negative correlation in resampling before clustering, which is non-significant in PRs.}
...
\textbf{accuracy is the most apparent case in PRs}, see the quartiles in table \@ref(tab:quartiles-f1-baseline).
...
\textbf{All applies to PRs as well.}
...
* \textbf{Accuracy and balanced accuracy} suggested that `no_resampling` was the \textbf{best strategy}, but this was confounded by the fact that it also held the \textbf{highest baselines}.
* \textbf{In PRs, MCC suggested resampling before clustering and semi resampling as the best strategies}.
* \textbf{In PRs, F1 and AUROC also suggested the no resampling strategy as the best one, but its baselines were not higher}.
...
A descriptive plot of the the adjusted metrics (figure \@ref(fig:adjperf-boxplot-strat-foldavg)) pointed to a \textbf{scenario different than that of unadjusted ones} (figure \@ref(fig:perf-boxplot-strat-foldavg)).
...
Again, `strategy` covariate was \textbf{always significant} in a type 3 ANOVA (table \@ref(tab:anova-adjperf-strat)). 
\textbf{Baseline adjustment brought a uniform behaviour across the models, except for AUROC still suggesting no resampling as the best strategy in PRs} (table \@ref(tab:mod-adjperf-absolute)), further confirmed in pairwise coefficient comparison (Tukey's method, figure \@ref(fig:adjperf-tukey-absolute-strat)) and in their expected performance (table \@ref(tab:emm-adjperf-strat) and figure \@ref(fig:adjperf-ranking-absolute-strat)):

* \textbf{In PRs, resampling before clustering was the best performer, closely followed by semi resampling, then no resampling, and resampling after clustering}. 
* \textbf{In PRs the general picture of resampling before clustering and semi resampling being the best was already present in unadjusted MCC, and is mostly kept in ajdusted metrics}. 
...
* \textbf{In PRs, resampling before clustering performed similarly to semi resampling, and better than no resampling and resampling after clustering. Therefore, augmenting the test set was not the largest performance driver}.
* `semi_resampling` outperformed `no_resampling` in \textbf{two out of five metrics, and non-significant in the remaining three} (Tukey's method, $p < 0.05$, figure \@ref(fig:adjperf-tukey-absolute-strat)), which **supports data augmentation usefulness** even if the data balance in the test set differs from that of the training set. This was \textbf{consistent with the observation that the predicted proportion of positives of the PCM model was mainly driven by the actual data balance in the test set, rather than that of the training set. Combined with the healthier distributions of predicted active ratios} of `semi_resampling` against `no_resampling`, this made a \textbf{case in favour of the former}.
