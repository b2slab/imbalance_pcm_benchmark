---
title: "Appendix 4: model predictions and performance (NRs)"
author: 
...
df.clean <- read.csv(config$file.cleanperf.nr) %>%
...
This supplement describes the behaviour of the proteochemometrics (PCM) deep learning model to predict protein-compound bioactivity for \textbf{NRs}. 
...
* `no_resampling` \textbf{keeps similar data imbalance in training and test.}
* `resampling_before_clustering` and `semi_resampling` \textbf{lead to a more balanced training set, but not so much for the test set.}
* `resampling_after_clustering` \textbf{kept balanced proteins} in both training and test sets.

In addition, \textbf{test sets with imbalance tended to magnify} it and create extreme cases (all actives or all inactives), probably due to the combination of the clustering and the lower sample sizes in the test sets compared to training.
...
* `no_resampling` showed a \textbf{positive relation} between both, i.e. proteins were prone to keep their (im)balance in train and test.
* `resampling_before_clustering` showed an \textbf{inverse relationship} instead. This was expected since this strategy started from globally balanced proteins, and after the clustering, an imbalance in one direction in the training set entailed an inverse imbalance in the test set.
* `semi_resampling` led to \textbf{independent train and test balances, expected since the train set was resampled, breaking any correlation with the test set balance.} 
* `resampling_after_clustering` always \textbf{kept balanced proteins}, by design.
...
* \textbf{Proteins with greatest imbalance} (i.e. where `(0.5 - ratio_test)^2` was greatest) \textbf{tended to be among those with the least interactions. Linear correlations were significant} (table \@ref(tab:cortest-ninter-original)).
* \textbf{The sequence length had no obvious effect on the protein imbalance. Linear correlations were not significant} (`resampling_before_clustering`) or significant but low (`no_resampling`, `semi_resampling`).
...
* `no_resampling` was noticeably \textbf{biased to predict everything as positives}.
* `resampling_before_clustering` and `semi_resampling` \textbf{alleviated the imbalance in the predictions, but still retained a spike of proteins where all the compounds were predicted as positives}.
* `resampling_after_clustering` \textbf{kept a wide and symmetric distribution of predicted actives}.
...
Now, representing together (1) the original training and test ratios, and (2) the predicted ratios in test (figure \@ref(fig:ratio-hist-all)) eased a general qualitative assessment: \textbf{the distribution was most resemblant to that of the test proportions to that of the training ones} (except `resampling_after_clustering`, since those proportions are constant). Table \@ref(tab:perc-extremes) displays how `no_resampling` was \textbf{highly inclined to predict all positives}, `resampling_before_clustering` and `semi_resampling` \textbf{alleviated this phenomenon}, and `resampling_after_clustering` was \textbf{essentially balanced}.
...
* `no_resampling`: \textbf{positive trend between the training and the predicted ratio}, but since the training and the test ratio also positively correlated (figure \@ref(fig:ratio-scatter-original)), the latter could be the one driving the predicted ratio of positives.
* `resampling_after_clustering` had a \textbf{constant training ratio}, meaning that the predicted ratio was not explainable by differences in training ratios.
* `resampling_before_clustering` showed instead a \textbf{negative relation between the training and the predicted ratio}. But since the former and the test ratio also anticorrelated (figure \@ref(fig:ratio-scatter-original), the simplest explanation was that the test ratio drove the predicted test ratio.
* `semi_resampling` showed \textbf{no apparent correlation} between the predicted ratio and the training ratio.
...
ggplot(df.clean.nobaseline, 
  ...
  facet_wrap(~strategy, nrow = 1) +
  xlab("Actives ratio (training set)") +
  ylab("Predicted actives ratio (test set)") +
  theme(aspect.ratio = 1, legend.position = "none")
```
...
\textbf{Due to the lack of correlation between training and test ratios} (figure \@ref(fig:ratio-scatter-original)), the `semi_resampling` strategy is the \textbf{ideal scenario to disentangle their effects} on the predicted ratio of actives (see model in table \@ref(tab:mod-prop-semi-before)).
This additive model suggests:

* The \textbf{test ratio is driving the predicted proportions, rather than the training ratio}.
* `n_interactions`: \textbf{the term is not significant in NRs}.
...
* \textbf{This model confirms both conclusions from the model} in the `semi_resampling` strategy, with \textbf{similar estimates.} 
...
The explanatory linear model under the `no_resampling` strategy (table \@ref(tab:mod-prop-nores)) \textbf{suffers from the positive correlation between training and test ratios, which can be confounded}.

* Both `training_ratio` and `test_ratio` show a \textbf{positive effect} on the predicted fraction of actives. 
* Although the \textbf{estimate is larger and more significant} for `training_ratio`, the \textbf{confounding effect and the very skewed distribution of the predicted ratios deems this model inconclusive}.
...
\textbf{All of them apply to NRs.}
...
\textbf{Visual inspection suggested that accuracy, F1 and possibly balanced accuracy were affected by the data imbalance}.
\textbf{F1 is the most apparent case}, see the quartiles in table \@ref(tab:quartiles-f1-baseline).
...
The `strategy` variable was tested with a type 3 ANOVA, being \textbf{significant} with $p < 0.05$ for `acc`, `f1` and `balanced_acc` (table \@ref(tab:anova-baseline-strat)). 
...
\textbf{All applies to NRs as well.}
...
This illustrates the \textbf{issue of direct strategy comparison} with imbalance-sensitive metrics, which was especially visible for the F1-score.
...
```{r perf-boxplot-strat-foldavg, fig.width=6, fig.height=5, fig.cap='Performance metrics for balancing strategies and their corresponding imbalance-aware random baselines. Data points correspond to proteins, averaged over folds.'}
...
ggplot(df.clean.foldavg.long, 
    ...
    geom_boxplot(outlier.size = .25, lwd = .25) +
    facet_wrap(~metric, scales = "free_y") +
    scale_fill_manual(values = config$col.strategy, guide = FALSE) +
    scale_colour_manual(values = c("gray5", "gray50"), name = "Prediction type") +
    scale_alpha_manual(values = c(1, .5), name = "Prediction type") +
    xlab("Strategy") +
    ylab("Performance") +
    theme(legend.position = c(.87, .15), aspect.ratio = 1, 
          axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
```
...
The `strategy` covariate was \textbf{always significant} in a type 3 ANOVA (table \@ref(tab:anova-perf-strat)). 
The models, summarized in \@ref(tab:mod-perf-absolute), showed \textbf{different behaviour in imbalance-sensitive and insensitive metrics}.
Pairwise comparisons of the strategy coefficients using Tukey's method would point to \textbf{two different pictures} (figure \@ref(fig:perf-tukey-absolute-strat)), further confirmed when prioritizing the strategies according to their expected performance through the linear models (table \@ref(tab:emm-perf-strat) and figure \@ref(fig:perf-ranking-absolute-strat)):

* \textbf{Accuracy, balanced accuracy and F1-score} suggested that `no_resampling` was the \textbf{best strategy}, but this was confounded by the fact that it also held the \textbf{highest baselines} .
* \textbf{AUROC and MCC} and showed instead that `resampling_before_clustering` held the \textbf{highest performance estimates}. 
...
A descriptive plot of the the adjusted metrics (figure \@ref(fig:adjperf-boxplot-strat-foldavg)) pointed to a \textbf{scenario diffent than that of unadjusted ones} (figure \@ref(fig:perf-boxplot-strat-foldavg)).
...
Again, `strategy` covariate was \textbf{always significant except for AUROC in NRs} in a type 3 ANOVA (table \@ref(tab:anova-adjperf-strat)). 
\textbf{Baseline adjustment brought a uniform behaviour across the models} (table \@ref(tab:mod-adjperf-absolute)), further confirmed in pairwise coefficient comparison (Tukey's method, figure \@ref(fig:adjperf-tukey-absolute-strat)) and in their expected performance (table \@ref(tab:emm-adjperf-strat) and figure \@ref(fig:adjperf-ranking-absolute-strat)):

* \textbf{In NRs}, `resampling_before_clustering` \textbf{was the best performer, followed by a technical tie between} `no_resampling` and `semi_resampling`, and `resampling_after_clustering`. 
* \textbf{In NRs the statistical significance is lower, but the general picture in the imbalance-insensitve non-adjusted metrics remains in the adjusted metrics: resampling before clustering being the best performer and no resampling being sensibly penalised }. 
...
```{r adjperf-ranking-absolute-strat, fig.width=8, fig.height=6, fig.cap='Method ranking according to the linear model predicted adjusted performances for each metric. Baseline metrics were taken into account in the adjustment. For a direct comparison, the same ranking using the absolute metrics was kept side by side.'}
df.metrics.both <- dplyr::bind_rows(
  mutate(df.metrics.absolute, modtype = "Absolute metrics"), 
  mutate(df.metrics.diff, modtype = "Baseline-adjusted metrics")
)

ggplot(df.metrics.both, aes(x = strategy, y = metric, fill = rescaled, label = label)) + 
  geom_tile(color = "white") + 
  geom_text(size = 2.75) + 
  # scale_fill_gradient2(mid = "white", high = "mediumpurple1") + 
  scale_fill_distiller(palette = "Spectral", direction = -1, name = "Performance rescaled by\neach block row maximum") +
  facet_wrap(~modtype, nrow = 1) +
  coord_fixed() +
  xlab("Strategy") + 
...
* \textbf{In NRs, resampling after clustering and semi resampling are technically tied, so data augmentation in the test set is not the largest performance drive}. 
* \textbf{In NRs, the almost six-fold reduction in number of samples compared to kinases probably limited the statistical power when comparing semi resampling and no resampling}. Tukey's method gave no significant differences at $p < 0.05$. However, the model coefficient of semi resampling was significantly greater than the reference level (no resampling) in MCC ($p = 0.033$), but not in balanced accuracy ($p = 0.067$) or accuracy ($p = 0.111$), despite the positive estimates}, see table \@ref(tab:mod-adjperf-absolute). This was \textbf{consistent with the observation that the predicted proportion of positives of the PCM model was mainly driven by the actual data balance in the test set, rather than that of the training set. Combined with the healthier distributions of predicted active ratios} of `semi_resampling` against `no_resampling`, this made a \textbf{case in favour of the former}.
* In \textbf{five} out of five metrics, **proteins with more interactions were better predicted** (table \@ref(tab:mod-adjperf-absolute)).
