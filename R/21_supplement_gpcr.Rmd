---
title: "Appendix 3: model predictions and performance (GPCRs)"
author: 
...
This supplement describes the behaviour of the proteochemometrics (PCM) deep learning model to predict protein-compound bioactivity for \textbf{GPCRs}. 
...
* `semi_resampling` led to \textbf{a slight positive correlation, but weaker than no resampling}.
...
* \textbf{The sequence length had no obvious effect on the protein imbalance. Linear correlations were not significant} (`no_resampling`, `semi_resampling`) or significant but low (`resampling_before_clustering`). 
...
\textbf{Due to the only slight correlation between training and test ratios} (figure \@ref(fig:ratio-scatter-original)), the `semi_resampling` strategy is the \textbf{ideal scenario to disentangle their effects} on the predicted ratio of actives (see model in table \@ref(tab:mod-prop-semi-before)).
This additive model suggests:

* The \textbf{test ratio is driving the predicted proportions, rather than the training ratio}.
* `n_interactions`: \textbf{the term is not significant in GPCRs}.
...
* \textbf{This model confirms both conclusions from the model} in the `semi_resampling` strategy. \textbf{The ratio in training is also significant for GPCRs, but lower significance and estimate than the ratio in test; number of interactions slightly significant now.}
...
\textbf{All of them apply to GPCRs.}
...
\textbf{All applies to GPCRs as well.}
...
* Accuracy and F1-score suggested that `no_resampling` was the \textbf{best strategy}, but this was confounded by the fact that it also held the \textbf{highest baselines}.
* AUROC, MCC and balanced accuracy showed instead that `resampling_before_clustering` and `resampling_after_clustering` held the \textbf{highest performance estimates}. 
...
* `resampling_before_clustering` and `resampling_after_clustering` had the \textbf{highest performance estimates, followed by} `semi_resampling` and finally by `no_resampling`.
* This \textbf{picture was analogous to that of the non-adjusted performance metrics that were imbalance-insensitive} (AUROC, MCC). 
