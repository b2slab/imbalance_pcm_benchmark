...
lapply(list.lm.strat.diff, summary)
```
...
# Analysis of protein performance

## F1-score, baseline-adjusted

```{r}
df.prot.diff.scatter <- tidyr::pivot_longer(
  df.clean.diff.nobaseline, 
  cols = config$v.metrics, 
  names_to = "metric", 
  values_to = "performance") %>%
  dplyr::select(DeepAffinity.Protein.ID, strategy, fold, metric, performance) %>%
  tidyr::pivot_wider(
        id_cols = c("DeepAffinity.Protein.ID", "fold", "metric"),
        names_from = "strategy", values_from = "performance") %>%
  ungroup
```

```{r, fig.width=7, fig.height=10}
ggplot(df.prot.diff.scatter, aes(x = no_resampling, y = semi_resampling)) +
...
Aggregate over folds, rank by metric

```{r}
# rank metrics DECREASINGLY, keep NAs as NAs
# and rescale to lie in [0, 1] 
# otherwise some ranks will not be comparable for different lengths..
rank_na <- function(x) {
  ind.na <- is.na(x)
  rk <- rank(-x[!ind.na])
  x[!ind.na] <- rk/max(rk)
...
df.prot.diff.rank <- group_by(df.prot.diff.scatter, fold, metric) %>%
  mutate_if(is.numeric, rank_na)
```

```{r, fig.width=5, fig.height=5}
tidyr::pivot_longer(df.prot.diff.rank, cols = config$v.strategy, 
                    names_to = "strategy", values_to = "performance") %>%
  group_by(fold, metric, DeepAffinity.Protein.ID) %>%
  mutate(performance_avg = mean(performance, na.rm = TRUE)) %>%
  ggplot(aes(x = performance_avg)) +
  geom_histogram() +
  facet_wrap(~strategy) +
  theme_bw()
...
tidyr::pivot_longer(df.prot.diff.rank, cols = config$v.strategy, 
                    names_to = "strategy", values_to = "performance") %>%
  group_by(strategy, metric, DeepAffinity.Protein.ID) %>%
  summarise(performance_avg = mean(performance, na.rm = TRUE)) %>%
  ggplot(aes(x = performance_avg)) +
  geom_histogram() +
